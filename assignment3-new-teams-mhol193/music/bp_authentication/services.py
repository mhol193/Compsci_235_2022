...
def add_user(user_name: str, password: str, repo: AbstractRepository):
    ...
    # Create and store the new User, with random dp and password encrypted.
    photos_dir = pathlib.Path.cwd() / 'music' / 'static' / 'profile_photos'
    list_of_photos: list = os.listdir(photos_dir)
    list_of_photos.remove('default.png')
    rand_index = random.randrange(len(list_of_photos))
    user = User(user_name, password_hash, list_of_photos[rand_index])
    repo.add_user(user)
