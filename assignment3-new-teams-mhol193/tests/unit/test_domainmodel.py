...
class TestReview:
    ...
    def test_construction(self):
        ...
        user = User("user1", "user1", profile_photo='bear.png')
        review1 = Review(track1, 'Sample review', 3, user)
        review2 = Review(track1, ' Another review ', 2, user)
    ...
    def test_attributes_fail(self):
        ...
        review1 = Review(track1, 'Sweater Weather', 2, user)
    ...
    def test_equality(self):
        ...
        review1 = Review(track1, 'Sweater Weather', 2, user)
        review3 = Review(track1, 'Sweater Weather', 5, user)
        review4 = Review(track1, 'Sweater Weather 2', 2, user)
        review5 = Review(None, 'Sweater Weather', 2, user)

        # reviews should be considered equal as long track and user match
...
class TestUser:
    ...
    def test_construction(self):
        user1 = User('amotys', 'amotys277', 'bear.png')
        user2 = User('  yunwi5  ', 'urrabbit978', 'bear.png')

        assert str(user1) == '<User amotys>'
        assert str(user2) == '<User yunwi5>'

        # passing int as user_name raises error
        with pytest.raises(ValueError):
            User(10, 'pedri1928', 'bear.png')

        # Test user_name is all lowercase
        user3 = User(' GAVI ', 'gavi1928', 'bear.png')
        assert user3.user_name == 'gavi'

        # Invalid password type set password to None
        user5 = User('Memphis', 325, 'bear.png')
        assert user5.password is None

        user5 = User('Memphis', '', 'bear.png')
        assert user5.password is None

        # Password length < 8 sets password to None
        user5 = User('memphis', 'mempi', 'bear.png')
        assert user5.password is None

        # Password of length 8 sets the password correctly
        user6 = User('Memphis', 'mempi123', 'bear.png')
        assert user6.password == 'mempi123'
    ...
    def test_attributes_fail(self):
        ...
        with pytest.raises(AttributeError):
            user1.profile_photo = "random.png"
    ...
    def test_equality(self):
        user1 = User('amotys', 'amotys277', 'bear.png')
        user1_copy = User('amotys', 'amotys277', 'cat.png')
        user2 = User('gavi', 'gavi9281', 'bear.png')
        user3 = User('phil', 'phi8901', 'bear.png')
    ...
    def test_track_methods(self):
        ...
        user1 = User('pedri', 'pedri9281', 'bear.png')
        track1 = Track(1, 'Shivers')
        track2 = Track(2, 'Heat Waves')
        track3 = Track(3, 'Bad Habit')
        ...
        user2 = User('thorke', 'pedri9281', 'bear.png')
        user3 = User('fmerc', 'pedri9281', 'bear.png')
        user1.add_request(user2)
        user1.add_request(user3)
        assert user1.requests == [user2, user3]
        user1.add_request(user2)
        assert len(user1.requests) == 2
        with pytest.raises(ValueError):
            user1.add_request(2)
        assert len(user1.requests) == 2
